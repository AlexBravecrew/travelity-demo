...
- **Start Free Trial site-wide → `https://admin.travelity.app`**: new `Paths.START_TRIAL` constant. All four Start Free Trial CTAs (Nav, MobileMenu, Hero, ProductHero, ClosingCTASection) point at it with `external={true}` (target=_blank + rel=noopener noreferrer).
